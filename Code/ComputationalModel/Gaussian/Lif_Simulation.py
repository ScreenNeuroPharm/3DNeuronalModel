...
should_save = False         # False=do not save output  -   True=save output
should_show = True          # False=do not show figures
DateFolder = str(date.today())   # Date in the format aaaa-mm-dd
SimulationNumber = 1      # Number of simulation in the date
...
# L0 to L1; L0 to L2; L0 to L3; L1 to L2; L1 to L3; L2 to L3
# L1 to L0; L2 to L0; L2 to L1; L3 to L0; L3 to L1; L3 to L2
...
# ------ RASTER shuffle ------
fig2 = figure(2, figsize=[5 * num_layers * cm_fig_conv, 6 * cm_fig_conv], dpi=600.0)
# fig2 = figure(2, figsize=[6.6 * cm_fig_conv, 3.3 * cm_fig_conv], dpi=600.0)
ax0 = subplot(1, num_layers, 1)
pt_l0 = sm_layer0.spike_trains()
for key, __ in pt_l0.items():
    plot(pt_l0[key], repeat(l0.index[key], len(pt_l0[key])), ',k')
xlim(preptime / second, (preptime + simtime) / second)
xlabel('Time (s)')
if num_layers > 1:
    ...
    for key, __ in pt_l1.items():
        plot(pt_l1[key], repeat(l1.index[key], len(pt_l1[key])), ',k')
    xlim(preptime / second, (preptime + simtime) / second)
    xlabel('Time (s)')
if num_layers > 2:
    ax2 = subplot(1, num_layers, 3, sharex=ax0)
    ...
    for key, __ in pt_l2.items():
        plot(pt_l2[key], repeat(l2.index[key], len(pt_l2[key])), ',k')
    xlim(preptime / second, (preptime + simtime) / second)
    xlabel('Time (s)')
if num_layers > 3:
    ...
    for key, __ in pt_l3.items():
        plot(pt_l3[key], repeat(l3.index[key], len(pt_l3[key])), ',k')
    xlim(preptime / second, (preptime + simtime) / second)
    xlabel('Time (s)')
if num_layers > 4:
    ...
    for key, __ in pt_l4.items():
        plot(pt_l4[key], repeat(l4.index[key], len(pt_l4[key])), ',k')
    xlim(preptime / second, (preptime + simtime) / second)
    xlabel('Time (s)')
if num_layers > 5:
    ...
    for key, __ in pt_l5.items():
        plot(pt_l5[key], repeat(l5.index[key], len(pt_l5[key])), ',k')
    xlim(preptime / second, (preptime + simtime) / second)
    xlabel('Time (s)')
ylabel('Neuron index (position)', rotation='vertical')
gca().spines['top'].set_visible(False)
