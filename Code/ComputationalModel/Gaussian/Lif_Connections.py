...
def conneting_layers(pop_source: NeuronGroup, exc_source: NeuronGroup, inh_source: NeuronGroup, 
    ...
    """
      ...
      :return: s_exc_all, s_inh_exc
      """
    s_exc_all = connecting_3D(exc_source, pop_target, 'I_synEX+=alpha_ex * w', sigma, sigma_3D, V_ax_P, p_max, 1000 * mV)
    s_inh_exc = connecting_3D(inh_source, exc_target, 'I_synIN-=alpha_in * w', sigma, sigma_3D, V_ax_I, p_max, 1000 * mV)
    ...
    for idx in range(ll):
        ...
        if s_exc_all.N_outgoing_pre[idx] > max_conn_ext_exc[idx]:
            ...
            k = count - diff
            r = []
            r = random.sample(range(s_exc_all.N_outgoing_pre[idx]), diff)  # prendo tanti indici quanto le connessioni da mettere a peso zero
            for index1 in range(len(r)):
    ...
    for idx in range(ll):
        ...
        if s_inh_exc.N_outgoing_pre[idx] > max_conn_ext_inh[idx]:
            ...
            k = count - diff
            r = []
            r = random.sample(range(s_inh_exc.N_outgoing_pre[idx]), diff)  # prendo tanti indici quanto le connessioni da mettere a peso zero
            for index1 in range(len(r)):
...
if __name__ == '__main__':
    ...
    plot(l0_exc.x[neuron_idx_exc] / umeter, l0_exc.y[neuron_idx_exc] / umeter, 'o', mec='g', mfc='g')
    scatter(l0.x[s_exc_all_l0.j[neuron_idx_exc, :]] / umeter, l0.y[s_exc_all_l0.j[neuron_idx_exc, :]] / umeter,
            s=((s_exc_all_l0.w[neuron_idx_exc, :] / mV) * 5)**6, marker='.', c='r')
    title('exc')
    xlabel('x (µm)')
    ...
    plot(l0_inh.x[neuron_idx_inh] / umeter, l0_inh.y[neuron_idx_inh] / umeter, 'o', mec='g', mfc='g')
    scatter(l0_exc.x[s_inh_exc_l0.j[neuron_idx_inh, :]] / umeter, l0_exc.y[s_inh_exc_l0.j[neuron_idx_inh, :]] / umeter,
            s=((s_inh_exc_l0.w[neuron_idx_inh, :] / mV) * 5)**6, marker='.', c='b')
    title('inh')
    xlabel('x (µm)')
    ylabel('y (µm)', rotation='vertical')
    gca().spines['top'].set_visible(False)
